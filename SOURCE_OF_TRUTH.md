...
- `training/canonical_d64.py`, `training/complete_field_64d.py`, and `training/train_complete_field_64d.py` ? developmental canonical D64 reasoning path; `training/heart_translation.py` plus `scripts/train_heart_translation_smoke.py` ? first provenance-bound Heart translation curriculum/evaluation and Trainer-governed bounded candidate smoke path with content-addressed task objective and no activation,
