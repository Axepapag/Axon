...
`training/substrate_literacy_curriculum.py` is the first developmental curriculum. It teaches stable substrate identity and use before open-ended reasoning: the complete frozen native symbol bank, exact sequences, alphabet/digit adjacency, spaces/newlines, punctuation, case and arbitrary Unicode compositions, all through real canonical fields and exact D64 source alignment. FIRST and REFINED are supervised at this stage so the single Core externalizes a substrate reading, re-attends its own proposal, and emits it again. The consolidator pass still occurs and advances private Soul, but tagged FINAL text is not yet a Stage-0 optimization requirement. Stage-0 mastery is exact held-out text, exact teacher-forced content and EOS, complete-field coverage, and performance above the strongest constant-output floor.

Every training Core owns HOT/WARM/COLD/DEEP_COLD from birth. Candidate training does not reset Soul between examples or optimizer steps. A bounded optimizer step keeps parameters fixed while a short ordered lived trajectory runs; each experience executes FIRST, REFINED and CONSOLIDATED Soul transitions, and the next experience inhales the exact previous successor Soul. Only after the trajectory loss is accumulated does the optimizer mutate weights. The accepted parameter checkpoint and every Soul transition are then committed as one governed step bundle. Initial Stage-0 execution uses eight lived experiences per optimizer step. Gradient history may be truncated at optimizer boundaries; Soul lineage is not. Early substrate education updates HOT while the colder layers remain present and preserved for later compaction/distillation mechanisms.
