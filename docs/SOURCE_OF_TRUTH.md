...
**2026-09-19 scratch-substrate launch amendment:** the first current English-native developmental run starts from fresh parameters; the step-720 donor bridge is preserved only as an optional governed experiment and is not used by this lineage. The baseline Core is `d_model=64` with one 64D attention head. Its first bounded local tranche uses two Transformer layers, `ffn_dim=16384`, four persistent state tokens, 32-character processing pages, neutral copy/generate bias, and no retired decision/operation/address or special termination heads. EOS is ordinary generated text termination: its probability comes from the generated text distribution and is not permission-gated by the content copy/generate router. The active config rejects the old receipt-continuation/EOS-route/termination-head switches.

`training/substrate_literacy_curriculum.py` is the first developmental curriculum. Stage-0A now teaches only exact substrate identity and transport before asking the Core to reason about symbol relations: the complete frozen native symbol bank, mixed variable-length exact sequences, spaces/newlines/tabs, punctuation, case and Unicode compositions, all through real canonical fields and exact D64 source alignment. Before/after/index reasoning is deferred until exact reading and termination are learned. FIRST and REFINED are supervised during training for credit assignment, but heldout evaluation must use the Core's actual free-running FIRST proposal as the REFINED proposal-board input; authored correct workspace text is never mastery evidence. The consolidator pass still occurs and advances private Soul, but tagged FINAL text is not yet a Stage-0A optimization requirement. Stage-0A mastery is exact held-out text, exact teacher-forced content and EOS, complete-field coverage, and performance above the strongest constant-output floor.

Every training Core owns HOT/WARM/COLD/DEEP_COLD from birth. Candidate training does not reset Soul between examples or optimizer steps. A bounded optimizer step keeps parameters fixed while a short ordered lived trajectory runs; each experience executes FIRST, REFINED and CONSOLIDATED Soul transitions, and the next experience inhales the exact previous successor Soul. Only after the trajectory loss is accumulated does the optimizer mutate weights. The accepted parameter checkpoint and every Soul transition are then committed as one governed step bundle. Initial Stage-0A execution uses eight lived experiences per optimizer step. Gradient history may be truncated at optimizer boundaries; Soul lineage is not. Early substrate education updates HOT while the colder layers remain present and preserved for later compaction/distillation mechanisms.

**2026-09-20 Stage-0 evidence/evaluator correction:** the first fresh English-native CUDA tranche (`english-candidate-53f4ad04ba58c4d27348`) completed 64 optimizer steps / 512 lived experiences but did not pass substrate mastery: heldout exact was 0/24, teacher-forced content 8/118, teacher-forced EOS 11/24, complete-field coverage 100%, and no promotion was attempted. That run is diagnostic evidence only and does not parent the corrected Stage-0A curriculum merely because execution completed. The corrected v2 curriculum contains 267 train and 32 exact-text-disjoint heldout experiences and uses ordinary generated EOS with explicit loss weight 4.0. No learning claim exists for v2 until a new optimizer run is actually evaluated.

Heldout grading now executes the production-facing three-pass seam: real `ReasoningPassRequest`, real `emit`, real proposal-board rendering, and validated private-Soul transitions. REFINED therefore attends the actual FIRST output (or the actual failure/timeout accounting row), not an authored gold proposal. Failed/timed-out passes do not advance Soul. Mastery gates reject missing, nonnumeric, nonfinite, or out-of-range probability metrics. Trainer resume recovery runs under the writer lease before choosing the accepted parameter/Soul boundary. Each completed heldout evaluation is preserved as immutable content-addressed evidence tied to the exact accepted bundle and Soul; a mastery landmark may be written only when the gate actually passes.

Private-Soul usefulness remains a behavioral claim, not an inference from state continuity or latent norm changes. `training/soul_delayed_recall_probe.py` defines the current causal probe: arbitrary heldout cue/reply bindings are experienced, neutral ticks intervene, the reply is absent from the later field, and identical-weight recall is compared across intact, reset, swapped, and irrelevant private histories. A Soul-memory claim requires content-specific free-running behavior that passes those controls; the existence of the probe is not itself evidence that the current Core remembers.

The D64/1x64 baseline is an empirical starting anatomy, not a doctrine that every Core must share. Axon's intended ensemble remains heterogeneous; later Core widths/head layouts may be trained independently against the same canonical substrate and English proposal protocol.
