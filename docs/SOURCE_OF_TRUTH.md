...
- `runtime/trainer/` ? permanent Trainer parameter-authority anatomy: heterogeneous parameter+buffer inventory, OS single-writer lease, scoped mutation grants, isolated candidate optimizer execution, per-parameter telemetry, hash-bound checkpoint/restore, deterministic promotion gates, atomic active-generation pointers, exact activation/rollback snapshots and receipts, restart hydration, immutable lifecycle records, and read-only inspection; no model is trained or activated without an explicit governed plan/gate/activation path;
