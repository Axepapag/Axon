...
Updated: 2026-08-23T17:50:19-05:00
Current through event: `evt-20260823T225019230905Z-chatgpt-trainer-activation-rollback`
...
Trainer remains the primary organism focus and now owns governed candidate learning plus governed live-generation activation/rollback. `runtime/trainer/` enforces an OS-backed single parameter writer, complete heterogeneous parameter + persistent-buffer inventory, exact grant/plan lineage, isolated candidate cloning, grant-scoped AdamW/SGD execution, finite loss/gradient checks, per-step parameter telemetry, exact unauthorized/live-state hash checks, immutable lifecycle receipts, SHA256-bound checkpoint/restore, deterministic capability/counterfactual/regression/forgetting gates, and read-only inspection. The registered live organ is never handed to an optimizer, and a passed gate still cannot self-activate. Only the leased deterministic Trainer may advance the live generation after stale-inventory/checkpoint/proposal/gate checks, exact candidate value verification, a durable rollback snapshot and atomic active-pointer publication. Candidate `requires_grad` freezes remain training policy and are not imported into live runtime trainability. Full active verification is **315/315 tests passed**. Real `D:\Axon\State` Trainer inspection remains empty, including zero active-generation pointers, generation snapshots, activation receipts and rollback receipts (no writer lease, inventory, plan, candidate, optimizer step, checkpoint, evaluation, gate or promotion), so no semantic/reasoning training campaign was started.
...
- Root `SOURCE_OF_TRUTH.md` is an exact mirror. Current verified SHA256: `5CBDF500039EBC08722E8EECA7F5EC8E29A485399B0D47D9B6FB7B39416342E8` (Trainer candidate activation/rollback + atomic active-generation pointer doctrine).
...
4. **Primary focus remains the Trainer organ before new core training.** The isolated candidate lifecycle, optimizer boundary, writer lease, telemetry, checkpoint/restore, deterministic promotion gates, atomic live-generation activation/rollback, restart hydration and read-only inspection are now established. Next deepen **learning policy** (optimizer/scheduler/weight-decay/AMP/gradient-accumulation contracts and receipts), then make LoRA/adapters first-class constructed/attached/composed/retired generations and design provenance-preserving study/curriculum ingestion. No semantic/reasoning training campaign starts until those controls can govern it.
