...
Updated: 2026-08-23T19:02:44-05:00
Current through event: `evt-20260824T000244608585Z-chatgpt-lived-experience-doctrine`
...
Trainer remains the primary organism focus and now owns governed candidate learning, immutable learning-policy state, and governed live-generation activation/rollback. `runtime/trainer/learning.py` content-addresses the exact AdamW/SGD hyperparameters, weight decay, optimizer-specific settings, constant/warmup-cosine schedule, gradient accumulation, clipping, gradient/update L2 budgets and FP32/BF16/FP16 precision for each candidate. Mid-accumulation checkpoints preserve optimizer state, pending gradients, AMP scaler state when present, optimizer/microstep counters, accumulation index, current LR and accumulated loss sum for exact resume. The registered live organ is never handed to an optimizer, and only the leased deterministic Trainer may activate a fully gated candidate through the atomic active-generation pointer with exact rollback/restart recovery. Full active verification is **324/324 tests passed**; Trainer-focused+hygiene is **38/38**. Real `D:\Axon\State` Trainer inspection remains empty, including zero learning policies/microsteps, active-generation pointers, activation/rollback receipts, candidates, optimizer steps, checkpoints, gates or promotions, so no semantic/reasoning training campaign was started.

New lived-experience doctrine is now explicit: Dormant State is Axon's durable autobiographical memory as well as structured knowledge; masking is never deletion, and exact user/response/tool/advisor/canonical/outcome evidence must remain recoverable. Cortex semantically digests those episodes into provenance-bound concepts/relationships/procedures, while Trainer compiles selected successful/failed episodes into runtime-faithful curriculum so parameters learn procedural habit and intuition rather than serving as the only factual memory. The current Heart ingress journal is append-only but is **not yet** the complete Dormant autobiographical deposit path; implementing that path is now the next prerequisite before LoRA/adapter work.
...
- Root `SOURCE_OF_TRUTH.md` is an exact mirror. Current verified SHA256: `3F9DE8FEA284F44408A6E86B1E60BB4D8301FF1FF657C05FBB804FFF2F1C6973` (autobiographical continuity + lived-experience Trainer curriculum doctrine).
...
4. **Primary focus remains the Trainer organ before new core training, but the next gate has changed.** Candidate isolation, single-writer authority, immutable governed learning policies, AdamW/SGD execution, deterministic scheduling, accumulation, clipping/budgets, FP32/BF16/FP16 precision, exact resume, telemetry, promotion gates and activation/rollback are established. **Next: build the exact autobiographical Dormant experience-deposit path plus a provenance-complete lived-experience episode/curriculum compiler before LoRA/adapters.** Only after the Trainer can select and transform trustworthy life episodes should LoRA/adapters become first-class constructed/attached/composed/retired generations. No semantic/reasoning training campaign starts until those controls can govern it.
