...
authored it.
Master copy: `docs/WORKING_CONTRACT.md`.
...
## 12. Engineer's ledger (binding continuity record)

Every project turn, including read-only investigation, failed work, and
interrupted work, follows `roundtable/ENGINEERS_LEDGER_PROTOCOL.md`.

- Read `roundtable/ENGINEERS_LEDGER.md` before starting work.
- Record every material action in one new canonical turn event.
- Append that event to `roundtable/ENGINEERS_LEDGER_CANONICAL.jsonl` before
  ending the turn.
- Refresh the rolling summary through the new event.
- Never alter, delete, reorder, compact, or redact an existing canonical
  event. Corrections are appended as new events.

The rolling summary is continuity help. The canonical JSONL file is the
