...
class RemoteControlClient(
    ...
    data class RuntimeSummary(
        ...
        val fieldId: String?,
        val viewId: String?,
        val dormantIndexId: String?,
        val ingressQuarantineCount: Long?,
        val ingressRejectionCount: Long?,
    ...
    suspend fun runtimeSummary(): RuntimeSummary = withContext(Dispatchers.IO) {
        ...
        RuntimeSummary(
            ...
            fieldId = head?.optString("field_id")?.ifBlank { null }
                ?: heart?.optString("canonical_head_field_id")?.ifBlank { null },
            viewId = heart?.optString("last_view_id")?.ifBlank { null },
            dormantIndexId = heart?.optString("dormant_index_id")?.ifBlank { null },
            ingressQuarantineCount = heart?.optLongOrNull("quarantine_count"),
            ingressRejectionCount = heart?.optLongOrNull("rejection_count"),
